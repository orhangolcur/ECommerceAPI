...
builder.Services.AddHttpContextAccessor(); //Client'dan gelen request neticesinde oluþturulan HttpContext nesnesine katmanlardaki classlar üzerinde(business logic) eriþebilmemizi saðlayan bir servistir.
...
//Loglama ile ilgili yapýlandýrmayý burada yapýyoruz
...
// bu þekilde uygulamaya atýlan requestleri de loglayabiliriz.
...
builder.Host.UseSerilog(log); // bunu çaðýrdýðýmýz zaman default olan log mekanizmasýyla Serilog'u deðiþtirir 
...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme)
    .AddJwtBearer("Admin",options =>
    {
        options.TokenValidationParameters = new()
        {
            ...
            NameClaimType = ClaimTypes.Name // bu sayede kullanýcýnýn Name property'sine eriþebiliriz.
        };
    });
...
app.ConfigureExceptionHandler<Program>(app.Services.GetRequiredService<ILogger<Program>>()); // bu þekilde bir middleware oluþturup o metodu da burada kullanabiliriz.

app.UseStaticFiles(); //wwwroot'u kullanabilmek için çaðýrmamýz gerekiyor

app.UseSerilogRequestLogging(); //bunun altýndaki yapýlandýrmalar loglanýr
...
// username'i alabilmek için middleware yazdýk
